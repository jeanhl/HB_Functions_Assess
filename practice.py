...
def say_hi():
    print "Hi {}.".format(raw_input("What is your name? "))
...
def repeat_string(words, num_times):
    print "{}".format(words) * num_times
...
def total_meal_price(price, tip = .15):
    total = float(price) + float(price) * float(tip)
    return total
...
def write_letter(name, title, sender):
    print "Dear {} {}, I think you are amazing! Sincerely, {}".format(title,
           name, sender)


def add_new_number(x, list_of_nums):
